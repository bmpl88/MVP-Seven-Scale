...
export class AgentConsolidator {
  ...
  async processClientData(clientId) {
    try {
      ...
      // ✅ 3. Salva no Supabase com timestamp correto
      const savedInsight = await dbService.saveAgentInsights(clientId, insights, 'mvp-consolidator');
      
      logger.info(`✅ Insights generated and saved for client: ${clientId}`);
      console.log('📊 Saved insight details:', {
        client_id: clientId,
        processed_at: savedInsight?.processed_at,
