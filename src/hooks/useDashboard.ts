...
export function useDashboard() {
  ...
  const [persistenceLoaded, setPersistenceLoaded] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    // ✅ CORREÇÃO: Recuperar sincronizações dos clientes SEMPRE
    ...
    setPersistedClientSyncs(savedClientSyncs); // Definir mesmo se vazio
    ...
    // ✅ MARCAR PERSISTÊNCIA COMO CARREGADA
    setPersistenceLoaded(true);
    console.log('🏁 Persistência carregada e disponível para o Dashboard');
    }, []);

  /**
   * ✅ ESCUTAR EVENTOS DE ATUALIZAÇÃO DA PERSISTÊNCIA
   * Atualiza os dados quando localStorage é modificado
   */
  useEffect(() => {
    const handlePersistenceUpdate = () => {
      console.log('🔊 Evento de atualização da persistência detectado');
      
      // Recarregar dados persistidos
      const savedClientSyncs = persistence.getClientSyncs();
      setPersistedClientSyncs(savedClientSyncs);
      console.log('🔄 Dados persistidos atualizados:', savedClientSyncs.length, 'clientes');
    };
    
    window.addEventListener('sevenscale_persistence_updated', handlePersistenceUpdate);
    
    return () => {
      window.removeEventListener('sevenscale_persistence_updated', handlePersistenceUpdate);
    };
  }, [persistence]);
