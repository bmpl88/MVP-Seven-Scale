...
interface PersistedClientSync {
  clientId: string;
  lastSync: string;
  lastExecution?: string; // ✅ ADICIONAR SUPORTE PARA TIMESTAMP DINÂMICO
  timestamp: number;
}
...
export function usePersistence(config: PersistenceConfig = DEFAULT_CONFIG) {
  ...
  /**
   * ✅ SALVAR SINCRONIZAÇÕES DOS CLIENTES (COM SUPORTE A lastExecution)
   */
  const saveClientSyncs = useCallback((clientSyncs: Array<{ clientId: string; lastSync: string; lastExecution?: string; }>) => {
    try {
      const persistedData: PersistedClientSync[] = clientSyncs.map(sync => ({
        clientId: sync.clientId,
        lastSync: sync.lastSync,
        lastExecution: sync.lastExecution, // ✅ Preservar timestamp para cálculo dinâmico
        timestamp: Date.now()
      }));
